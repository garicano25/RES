        ...
        alertMensajeConfirm({
            ...
            await ajaxAwaitFormData({ api: 1, ID_FORMULARIO_INVENTARIO: ID_FORMULARIO_INVENTARIO }, 'InventarioSave', 'formularioINVENTARIO', 'guardarINVENTARIO', { callbackAfter: true, callbackBefore: true }, () => {
                ...
                $('.swal2-popup').addClass('ld ld-breath')
                    ...
                    // document.getElementById('formularioINVENTARIO').reset();
                    Tablainventarioinactivo.ajax.reload()
                    cargarMetricas();
...
var Tablainventarioinactivo = $("#Tablainventarioinactivo").DataTable({
      language: {
    ...
    ajax: {
        dataType: 'json',
        method: 'GET',
        url: '/Tablainventarioinactivo',
        beforeSend: function () {
            mostrarCarga();
        },
        complete: function () {
            Tablainventarioinactivo.columns.adjust().draw();
            ocultarCarga();
        },
    ...
    drawCallback: function () {
        ...
        const table = document.querySelector('#Tablainventarioinactivo');
...
$.fn.dataTable.ext.search.push(function (settings, data, dataIndex) {
    ...
    if (settings.nTable.id !== 'Tablainventarioinactivo') {
        return true;
    }

    if (!colorSeleccionado) return true;

    let row = Tablainventarioinactivo.row(dataIndex).node();
...
$('#Tablainventarioinactivo tbody').on('click', 'td>button.EDITAR', function () {
    var tr = $(this).closest('tr');
    var row = Tablainventarioinactivo.row(tr);
...
$(document).ready(function() {
    $('#Tablainventarioinactivo tbody').on('click', 'td>button.VISUALIZAR', function () {
        var tr = $(this).closest('tr');
        var row = Tablainventarioinactivo.row(tr);
...
$('#Tablainventarioinactivo tbody').on('change', 'td>label>input.ELIMINAR', function () {
    var tr = $(this).closest('tr');
    var row = Tablainventarioinactivo.row(tr);
    ...
    eliminarDatoTabla(data, [Tablainventarioinactivo], 'inventarioDelete');
});
...
function guardarRequiereItem() {
    ...
    .then(data => {
        ...
        if (data.code === 1) {
            Tablainventarioinactivo.ajax.reload(null, false);
        } else {
